window.addEventListener('DOMContentLoaded', (event) => {
    ...
    function renderPhoto() {
        const parentElement = document.getElementById('photo-list');
        const fragment = document.createDocumentFragment();
        if (photos) {
            photos.forEach((photo, index) => {
                let photoblock = document.createElement('div');
                photoblock.setAttribute('id', `content-${photo.id}`);
                let imgblock = document.createElement('img');
                imgblock.setAttribute('id', `img-${photo.id}`);
                imgblock.src = photo.thumbnailUrl;
    ...
    let start = 0;
    let end = 3;
    const photoList = document.querySelector('div.photo-list').children;
    // Initial Photos
    setActiveProp(start, end)
    function setActiveProp(start, end) {
        for (let index = 0; index < photoList.length; index++) {
            if (index >= start && index <= end) {
                photoList[index].classList.remove("inactive");
                photoList[index].classList.add("active");
            } else {
                photoList[index].classList.add("inactive");
                photoList[index].classList.remove("active");
            }
        }
